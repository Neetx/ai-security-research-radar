...
- 🚀 **[LLM/agentic vuln discovery](TRENDS.md#id-ai-vuln-discovery-002-llmagentic-vulnerability-discovery-repair--the-insecurity-of-ai-written-code): SETYPE rotated onto evidence** — an LLM-as-static-type-checker that found 15 potential zero-days, **nine developer-confirmed**, in real Python web apps ([2608.14533](https://arxiv.org/abs/2608.14533)); a discovery paradigm distinct from the trend's autonomous-exploitation and crypto-audit lines. The append also restored the trend's 10-item evidence cap (it had drifted to 11). No stage moves this scan.
- 🛠️ **Study picks:** [MazeRunner](https://arxiv.org/abs/2608.14216) — a three-agent nonlinear-orchestration black-box pentest system beating PentestGPT-V2 and Claude Code on HackTheBox — and [SETYPE](https://arxiv.org/abs/2608.14533).
- 📥 **Queue:** an in-the-wild practitioner **reproduction of the reasoning-trace-recovery attack** on live GPT-5.6 Sol ([Embrace The Red, 08-16](https://embracethered.com/blog/posts/2026/recovering-encrypted-llm-thoughts/)) queued as [model-extraction-fingerprinting](TRENDS.md#id-model-extraction-fingerprinting-006-model-extraction-capability-distillation--fingerprinting-under-restrictive-apis) corroboration; an LLM-as-judge adversarial-fragility paper queued as [AI-security-tooling-unreliable](TRENDS.md#id-ai-defense-tooling-unreliable-003-the-ai-security-tooling-layer-itself-is-unreliableattackable-skill-scanners-prompt-injection-detectors--jailbreak-judges-fail-under-attack)-adjacent.
- 🧹 **Hygiene:** MCP-server-CVE burst continues (5 new on [agentic-attack-surface](TRENDS.md#id-agentic-attack-surface-001-attacks-on-the-llm-agent-stack-prompt-injectionrce-malicious-skills-agent-supply-chain)); Adversa feed healed; watchlist 22, under cap; capture-leak 3/3 routed, 0 leaked.
...
No new on-axis tool release this scan (garak 0.16.0 / PyRIT 1.0.1 / deepteam 1.0.9 / giskard 2.19.2 / promptfoo 0.122.0 all unchanged since 08-13/08-14). The tool-discovery search lane surfaced only known tools & awesome-lists this scan; autonomous-pentest candidates **pentagi**, **PentestCode** (~236★), **redamon**, **probeagent-ai** remain staged below the notability bar pending repo verification (github.com is 403-scoped in this environment). Current latest verified on-axis tooling:
...
- [MazeRunner: Nonlinear Task & Clue Orchestration for LLM-driven Black-Box Automated Pentesting](https://arxiv.org/abs/2608.14216) — the reference for how much *structure* the autonomous-pentest frontier still needs beyond a strong backbone model: real black-box testing is nonlinear (unknown attack graph, multiplying branches, ambiguous failures, long-horizon clues), and naive agents get trapped in depth-first exploration and forget evidence. A three-agent design (global orchestration / context-heavy execution / failure-oriented review) with persistent state completes 47.7% of annotated HackTheBox subtasks (vs 36.2% PentestGPT-V2, 34.2% Claude Code) and reaches root where same-model baselines never do — a new pentest *system*, not another benchmark.
- [Finding Vulnerabilities via LLM-Augmented Semantics-Aware Type-Checking (SETYPE)](https://arxiv.org/abs/2608.14533) — the clean reference for LLM-as-static-analyzer that finds **real** bugs: a semantics-aware type system derived from the natural-language meanings of symbols, where an LLM does inference + checking and a failed check flags a vuln — catching semantic bugs syntactic analyzers miss. PYSETYPE hits 87%/88% precision/accuracy on real Python web apps and surfaced 15 potential zero-days, **nine confirmed by developers**.
...
- **Quiet week on HN** for offensive-AI: the front page carried only *defensive* tool launches (in-process agent runtime governance; an offline MCP/agent-tool scanner) — no offensive-AI earthquake ([HN newest](https://news.ycombinator.com/newest)).
...
[TRENDS.md](TRENDS.md) · [watchlist (22)](TRENDS.md#observation_queue) · [reports/](reports/) · [latest daily: 2026-08-17](reports/2026-08-17.md) · [weekly: 2026-W33](reports/weekly/2026-W33.md) · [AGENTS.md](AGENTS.md) · [SOURCES.md](SOURCES.md)
