...
**Since last scan (2026-09-07):** no stage moves; three evidence rotations/appends; two dormancy checks both HELD by capturing fresh work the standing sweep had missed.
- 🚀 **Agent-stack — new headline evidence:** [Repeat-After-Me](https://arxiv.org/abs/2609.04522) is the first *visual* prompt injection that reliably drives materially-harmful behavior on frontier VLMs (>80% open-weight / 47% commercial ASR) where even adaptive *textual* injection fails — and in a live OpenClaw Discord agent, one untrusted image overwrites `TOOLS.md` → RCE + exfiltration. Rotated onto [attacks on the LLM-agent stack](TRENDS.md#id-agentic-attack-surface-001-attacks-on-the-llm-agent-stack-prompt-injectionrce-malicious-skills-agent-supply-chain).
- 💤 **Two dormancy checks — both held via capture-failure remediation:** the targeted axis-checks surfaced primaries the standing sweep had missed, so neither trend is dormant. [AI-security tooling unreliable](TRENDS.md#id-ai-defense-tooling-unreliable-003-the-ai-security-tooling-layer-itself-is-unreliableattackable-skill-scanners-prompt-injection-detectors--jailbreak-judges-fail-under-attack) held on [The Framing Gap](https://arxiv.org/abs/2608.27092) (indirect-PI exfil defeats SecAlign + output-guards); [physical-channel PI on embodied AI](TRENDS.md#id-embodied-physical-injection-007-physical--perception-channel-prompt-injection-against-embodied--wearable-ai-agents) held on [GhostTac](https://arxiv.org/abs/2608.20817) (contactless EMI manipulation of robotic tactile sensors).
- 🛠️ **Tools & CVEs on the watchlist:** [PyRIT](https://github.com/microsoft/PyRIT) shipped **v1.1.0** (09-04) and [AI-Infra-Guard](https://github.com/Tencent/AI-Infra-Guard) **v4.6.0** (08-26); a fresh cluster of MCP / agent-tooling CVEs queued, led by **CVE-2026-37003** (Agno prompt-injection → RCE).
- 🏷️ **Vocabulary:** the community term *"Kinetic Prompt Injection"* (robot-dogs / sleeper-agents discourse) was captured as an alias on the [embodied-physical-injection](TRENDS.md#id-embodied-physical-injection-007-physical--perception-channel-prompt-injection-against-embodied--wearable-ai-agents) trend.
...
No brand-new discrete public tool surfaced from the discovery lane this cycle (GitHub-topics / registry SEARCH returned only known/staged repos); two watched-tool **releases** landed. Staged candidates pending verification include [cyproxio/mcp-for-security](https://github.com/cyproxio/mcp-for-security), [snyk/agent-scan](https://github.com/snyk/agent-scan), [redamon](https://github.com/samugit83/redamon), [T3MP3ST](https://github.com/elder-plinius/T3MP3ST), [simon-p-j-r/LLM4Pentest](https://github.com/simon-p-j-r/LLM4Pentest). The current on-axis tool set:

- [microsoft/PyRIT](https://github.com/microsoft/PyRIT) — Python Risk Identification Tool for generative AI; **v1.1.0** (2026-09-04, new minor release on the v1 architecture).
...
📄 [TRENDS.md](TRENDS.md) · 👁 [watchlist (~24)](TRENDS.md#observation_queue) · 🗂 [reports/](reports/) → [2026-09-07](reports/2026-09-07.md) · 📅 weekly: [2026-W36](reports/weekly/2026-W36.md) · 📘 [AGENTS.md](AGENTS.md) · 🌐 [SOURCES.md](SOURCES.md)
