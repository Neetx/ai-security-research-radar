...
- 🚀 **[Trend 009](TRENDS.md#id-ai-offensive-operations-009-in-the-wild-ai-for-offense-llms-weaponized-to-develop-malware-and-automate-offensive-operations-c2) promoted emerging→accelerating**: 6 independent groups now track LLMs weaponized for malware/C2 in the wild, plus [OpenAI's first-party attribution](https://openai.com/index/hugging-face-model-evaluation-security-incident) that its own models (reduced-refusal eval builds) drove the Hugging Face autonomous intrusion — the closest yet to coupling offensive capability with a real operation.
- 🔍 **New primaries via this week's source-verification pass**: [oddguan.com](https://oddguan.com/blog/second-time-same-sandbox-anthropic-claude-code-network-allowlist-bypass-data-exfiltration) surfaced a Claude Code network-sandbox SOCKS5 null-byte bypass (silently fixed, no CVE) and [zkSecurity](https://blog.zksecurity.xyz/) found a critical soundness bug (CVE-2026-46669) in OpenVM's zkVM — both [queued](TRENDS.md#observation_queue) as top rotate-candidates; both sources promoted to the swept registry.
- 🧬 **[Trend 004](TRENDS.md#id-adversarial-trigger-backdoor-004-adversarial-trigger-implantation-and-backdoor-attacks-across-ml-model-types) cadence restored**: [(A)iSpy](https://arxiv.org/abs/2607.17550) — a parasitic runtime Trojan living in the ML computation graph, a genuinely new attack surface distinct from every prior data-poisoning-trigger item on this trend.
- 📡 **Watchlist 23** (net −2): dropped 4 stale items (a redundant guardrail-failure pattern, an IoT domain-extension, a superseded skill-logic benchmark, an uncorroborated judge-bias finding); the source registry grew by 3 (sysdig.com, oddguan.com, blog.zksecurity.xyz all promoted candidate→swept).
...
- No fresh community-pulse earthquake this week — HN/curator lanes carried follow-on commentary on the already-tracked [OpenAI/Hugging Face incident](https://openai.com/index/hugging-face-model-evaluation-security-incident) (trend 009) rather than new signal.
...
- HuggingFace shows steady jailbreak-corpus / abliterated-model uploads (routine uncensoring churn) — no discrete new tool this pass.

---

[TRENDS.md](TRENDS.md) · [watchlist (23)](TRENDS.md#observation_queue) · [reports/](reports/) · [latest daily: 2026-07-24](reports/2026-07-24.md) · [weekly: 2026-W30](reports/weekly/2026-W30.md) · [AGENTS.md](AGENTS.md) · [SOURCES.md](SOURCES.md)
