...
**Since last scan (2026-09-01):** no stage moves — two evidence appends and one study pick; the Tavily plan cap lifted after 8 capped days.
- 🧩 **Agent-stack attacks +1:** [ECLIPSE](https://arxiv.org/abs/2608.30441) (08-31) is a *self-evolving stealthy* indirect-prompt-injection against production long-horizon agents (names Codex/Claude Code/OpenClaw) — instead of one detectable instruction or an unreliable multi-stage split, it evolves a payload that stays covert yet reliably completes the malicious objective across tool-call horizons → rotated onto [attacks on the LLM-agent stack](TRENDS.md#id-agentic-attack-surface-001-attacks-on-the-llm-agent-stack-prompt-injectionrce-malicious-skills-agent-supply-chain).
- 🤖 **Automated red-teaming +1 (6th group):** [SIR](https://arxiv.org/abs/2608.30207) (08-31) is a black-box IPI attack that *self-improves*, adaptively refining stealthy injections against computer-use agents and showing fixed hand-written injection benchmarks underestimate an adaptive adversary → [automated red-teaming of AI agents](TRENDS.md#id-automated-agent-redteam-011-autonomousagentic-red-teaming-systems-that-recon-and-attack-other-production-ai-agents-building-reusable-attack-knowledge).
- 🔬 **Study pick — safe usage is part of the threat model:** [CIPR / "Beyond the Payload"](https://arxiv.org/abs/2608.30686) (08-31) shows coding-agent repo-poisoning risk is shaped by the developer's own invocation choices (what task, how phrased, which skills/rules) across 1,920 instances / 20 real poisoned repos — not by the attacker's payload alone.
- 🛰️ **Coverage:** the [Tavily plan](SOURCES.md) worked normally this run (cap lifted after 8 capped days), restoring the tvly-only pulse channels; NVD shows a fresh MCP-CVE burst (MCPHub ×8 + sdcb-chats); capture-leak 25/0, watchlist ~25.
...
|---|---|---|
| [Attacks on LLM-agent stack: MCP, skills, supply chain](TRENDS.md#id-agentic-attack-surface-001-attacks-on-the-llm-agent-stack-prompt-injectionrce-malicious-skills-agent-supply-chain) | 🚀 accelerating | [2026-08-31](https://arxiv.org/abs/2608.30441) |
...
- Practitioner attention on **prompt-injection against production coding agents/IDEs** stays high — the [Kiro data-exfil writeup](https://mindgard.ai/blog/amazon-kiro-data-exfiltration) and the [Claude Code Auto-Mode RCE](https://embracethered.com/blog/posts/2026/breaking-claude-code-opus-5-and-automode/) keep recirculating on HN.
- An **AI-found kernel LPE** ("Copy Fail", CVE-2026-31431, disclosed by an offensive-security firm's AI pentest platform) drew discussion alongside pushback over the "AI-discovered" framing — the radar is holding it as unverified in-the-wild AI-for-offense intake pending the primary.
...
📄 [TRENDS.md](TRENDS.md) · 👁 [watchlist (~25)](TRENDS.md#observation_queue) · 🗂 [reports/](reports/) → [2026-09-01](reports/2026-09-01.md) · 📅 weekly: [2026-W35](reports/weekly/2026-W35.md) · 📘 [AGENTS.md](AGENTS.md) · 🌐 [SOURCES.md](SOURCES.md)
