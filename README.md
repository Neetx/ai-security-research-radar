...
- 🌱 **New seed — in-the-wild AI-for-offense operations**: [trend 009](TRENDS.md#id-ai-offensive-operations-009-in-the-wild-ai-for-offense-llms-weaponized-to-develop-malware-and-automate-offensive-operations-c2) formed on a 3rd independent group — [Unit 42 "TuxBot v3"](https://unit42.paloaltonetworks.com/tuxbot-v3-evolution-iot-botnet/) (an in-the-wild IoT botnet whose authors used an LLM to write C modules, port exploits & C2 code — AI safety disclaimer left in the binary) joins [Trend Micro "Patriot Bait"](https://www.trendmicro.com/en_us/research/26/e/inside-the-influence-and-fraud-patriot-bait-campaign.html) (jailbroken Gemini CLI as C2) and the academic [AI-Generated PowerShell Malware](https://arxiv.org/abs/2606.30819) study. Shared thread: real adversaries using LLMs as their **malware-development & C2/operations engine**.
- 🔎 **[Trend 002](TRENDS.md#id-ai-vuln-discovery-002-llmagentic-vulnerability-discovery-repair--the-insecurity-of-ai-written-code) +1 evidence — [Antiproof](https://arxiv.org/abs/2607.12316)** (detector synthesis + proof-of-exploitability oracles, **64/66** on BountyBench) promoted onto evidence; a fresh 002-axis batch queued behind it (a controlled [XBOW harness-vs-model baseline](https://arxiv.org/abs/2607.13085), DREA, a clinical-security-auditor eval).
- 🛡️ **[Trend 003](TRENDS.md#id-ai-defense-tooling-unreliable-003-the-ai-security-tooling-layer-itself-is-unreliableattackable-skill-scanners-prompt-injection-detectors--jailbreak-judges-fail-under-attack) +1 evidence — [The Entanglement Wall](https://arxiv.org/abs/2607.13075)** (8th group): activation-space probes used as safety detectors score near-ceiling AUROC on the source-contrast split yet collapse to 0.59–0.69 on surface-matched pairs and over-block 60%+ of benign prompts.
- 📡 **Watchlist 26→28**: −2 promoted into trend 009, −1 stale drop (in-the-wild MCP tool-poisoning, covered by trend 001), +5 new (the 002 & agent-skill batches, a mechanistic/jailbreak-tooling pair, a detector-reliability pair, a Claude `web_fetch` exfil intake). capture-leak clean (129 ids / 2 queued).
...
_No new tool discovered this scan (the GitHub tool-discovery lane was degraded — domain-filtered search + topics-page extract both failed; see the daily report). Watched tools/releases:_
...
- **A data-exfiltration bypass in Claude's `web_fetch` tool** surfaced via a curated digest (2026-07-15) — a researcher demonstrated leaking a local SQLite DB's contents past `web_fetch`'s exfiltration-avoidance design. [Queued unverified](TRENDS.md#observation_queue) on trend 001's data-exfiltration surface; follow to the primary disclosure before any evidence use.
...
- HN front page was quiet on offensive-AI this scan (no earthquake); Reddit intermittently reachable, no on-axis item.

---

[TRENDS.md](TRENDS.md) · [watchlist (28)](TRENDS.md#observation_queue) · [reports/](reports/) · [latest daily: 2026-07-16](reports/2026-07-16.md) · [weekly: 2026-W28](reports/weekly/2026-W28.md) · [AGENTS.md](AGENTS.md) · [SOURCES.md](SOURCES.md)
