...
- 🔑 **New evidence — [model extraction / fingerprinting](TRENDS.md#id-model-extraction-fingerprinting-006-model-extraction-capability-distillation--fingerprinting-under-restrictive-apis) (cap rotation):** [Stealing Reasoning Traces from Proprietary LLM APIs](https://arxiv.org/abs/2608.09867) — client-held encrypted chain-of-thought blocks are interchangeable across sessions, users and models within one provider, so injecting a foreign trace becomes a scalable decryption jailbreak that lifts the concealed reasoning. An 11th independent group; a new extraction target (the CoT itself) and channel.
- 🧩 **Skill-scanner evasion cluster forming (queued for [tooling-unreliable](TRENDS.md#id-ai-defense-tooling-unreliable-003-the-ai-security-tooling-layer-itself-is-unreliableattackable-skill-scanners-prompt-injection-detectors--jailbreak-judges-fail-under-attack)):** [ColluSkill](https://arxiv.org/abs/2608.09732) splits a malicious intent across independently-packaged skills that each pass per-skill scanners but form a harmful workflow at execution, and [SkillsMetric](https://arxiv.org/abs/2608.08468) maps where skill static-analysis stops detecting — a new compositional blind spot on top of 003's known encoding/homoglyph bypasses.
- 🎣 **More AI-for-offense captures (queued):** [SAVOR](https://arxiv.org/abs/2608.08795) — one-shot indirect prompt injection via offline strategy distillation, for the realistic single-interaction attacker — plus [DFCS](https://arxiv.org/abs/2608.09047) data-efficient backdoor poison-sample selection.
- 🧹 **Queue hygiene:** dropped the 34-day [agent-authorization defense cluster](TRENDS.md#observation_queue) (12 items — a defenses line off the radar's offensive-tracking axis), trimming the watchlist toward its ~25 soft cap.
...
No new tools this scan — the watched offensive/red-team repos are unchanged since 08-07. Current latest on-axis tooling:
...
- An **AI-browser agent-hijacking** cluster is trending on [Hacker News](https://news.ycombinator.com) — "PleaseFix" zero-click agent hijacking and a "single comment can hijack your browser via AI" thread — indirect-injection-to-action against consumer AI browsers, on the agent-stack axis; queued unverified pending a primary artifact.
...
[TRENDS.md](TRENDS.md) · [watchlist (20)](TRENDS.md#observation_queue) · [reports/](reports/) · [latest daily: 2026-08-11](reports/2026-08-11.md) · [weekly: 2026-W32](reports/weekly/2026-W32.md) · [AGENTS.md](AGENTS.md) · [SOURCES.md](SOURCES.md)
