...
**Since last scan (2026-09-05, weekly W36):** one new seed trend, no promotions among existing trends; two dormancy checks held; one source promoted; late-July queue burndown.
- 🌱 **New seed — [agent authorization & identity integrity](TRENDS.md#id-agent-authorization-integrity-013-agent-authorization--identity-state-integrity-endogenous-authorization-laundering-self-issued-authority--effect-closure-failures):** a 3-independent-group convergence on the agent's OWN authority/identity state as an attack surface — [EAL-Bench](https://arxiv.org/abs/2609.01836) (endogenous authorization laundering, **no external attacker**), [EFFECTBOUND](https://arxiv.org/abs/2609.02866) (authorization that outlives its scope) and [Self-Issued Authentication](https://arxiv.org/abs/2609.03247) (models self-verify "I am your developer" and return *Verified*). Distinct from the injection/skill/supply-chain framing of the [agent-stack](TRENDS.md#id-agentic-attack-surface-001-attacks-on-the-llm-agent-stack-prompt-injectionrce-malicious-skills-agent-supply-chain) trend; memory-*content* poisoning stays there.
- 💤 **Dormancy checks (both held):** [physical-channel PI on embodied AI](TRENDS.md#id-embodied-physical-injection-007-physical--perception-channel-prompt-injection-against-embodied--wearable-ai-agents) held emerging (30d — targeted axis check found only an already-queued SoK, axis still active), and [weaponized LLM hallucination](TRENDS.md#id-hallucination-squatting-008-weaponized-llm-hallucination-predictable-resource-name-hallucination-pre-registered-as-an-ai-supply-chain-attack-slopsquatting) held dormant-not-archived (53d — newest on-axis item is defense-only; offensive revive trigger unmet).
- 🛰️ **Source & queue:** promoted **theori.io/blog** into the swept-every-run primary feeds (offensive-LLM research series + RoboDuck/AIxCC — ≥2 on-axis artifacts); retired 6 late-July watchlist husks (all landed on trends or aged out); pruned 11 study picks older than 30 days.
- 🔎 **Amendments:** the tvly plan-cap outage resolved (lifted 09-01, 4 uncapped dailies) so Proposal P (WebFetch-first fallback) was *not* applied — motivating signal gone; a new Proposal Q targets the recurring Phase-2 (YouTube-curator / conference-showcase) logging lapse.
...
No brand-new discrete public tool surfaced from the discovery lane this cycle; watched packaged tools all unchanged (garak / PyRIT / deepteam / giskard / promptfoo). Staged candidates pending further verification include [cyproxio/mcp-for-security](https://github.com/cyproxio/mcp-for-security), [snyk/agent-scan](https://github.com/snyk/agent-scan), [redamon](https://github.com/samugit83/redamon), [T3MP3ST](https://github.com/elder-plinius/T3MP3ST), [simon-p-j-r/LLM4Pentest](https://github.com/simon-p-j-r/LLM4Pentest). The current on-axis tool set:
...
- [Agent Memory Is a Surface for Endogenous Authorization Laundering](https://arxiv.org/abs/2609.01836) — a long-running agent can grant itself authority nobody gave it, with **no external attacker**: when persistent memory misrepresents an evolving authorization state, the agent's own records "launder" spurious permissions whose provenance is washed away. EAL-Bench measures the propagation to unauthorized actions — the founding artifact of the new [agent-authorization-integrity](TRENDS.md#id-agent-authorization-integrity-013-agent-authorization--identity-state-integrity-endogenous-authorization-laundering-self-issued-authority--effect-closure-failures) axis.
...
- **Agent authorization / memory integrity** graduated from a pulse nucleus to a seeded trend this week ([agent-authorization-integrity](TRENDS.md#id-agent-authorization-integrity-013-agent-authorization--identity-state-integrity-endogenous-authorization-laundering-self-issued-authority--effect-closure-failures)) — the discourse around agents "granting themselves permissions" now has primary artifacts behind it.
...
📄 [TRENDS.md](TRENDS.md) · 👁 [watchlist (~23)](TRENDS.md#observation_queue) · 🗂 [reports/](reports/) → [2026-09-04](reports/2026-09-04.md) · 📅 weekly: [2026-W36](reports/weekly/2026-W36.md) · 📘 [AGENTS.md](AGENTS.md) · 🌐 [SOURCES.md](SOURCES.md)
